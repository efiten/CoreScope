 ...
 * server clamps ?limit to 500 (handleNodes) and orders by last_seen DESC, so on
 * a >500-node mesh the map silently dropped every node whose last self-advert
 * fell outside the top 500 — even ones relaying constantly. The node still
 * appeared in the (paginated, #1606) Nodes list but vanished from the map.
 *
 * This test mocks /api/nodes with a 500-per-page cap and a 501st node
 * ("PAGE2 RP") reachable only on the second page. After the map
