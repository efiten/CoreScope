...
// ===== Observer role support (#753 / PR #774) =====
{
  ...
  test('ROLE_COLORS.observer is defined and not empty', () => {
    const rolesJs = fs.readFileSync(REPO_ROOT + '/public/roles.js', 'utf8');
    const ctx = makeSandbox();
  ...
  test('Observer checkbox exists in neighbor graph filter section', () => {
    const analyticsJs = fs.readFileSync(REPO_ROOT + '/public/analytics.js', 'utf8');
...
// ===== #872 — hop-display unreliable badge =====
{
  ...
  function makeHopDisplaySandbox() {
    ...
    const hopSrc = fs.readFileSync(REPO_ROOT + '/public/hop-display.js', 'utf8');
...
// ===== #1504 — Path symbols legend disclosure =====
{
  ...
  vm.runInContext(fs.readFileSync(REPO_ROOT + '/public/hop-display.js', 'utf8'), ctx1504);
  ...
  test('#1504 regression: clicking <summary> in legend does NOT trigger column-sort handler', () => {
    ...
    const pktSrc = fs.readFileSync(REPO_ROOT + '/public/packets.js', 'utf8');
...
{
  const mapSrc = fs.readFileSync(REPO_ROOT + '/public/map.js', 'utf8');
