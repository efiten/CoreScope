    ...
    try {
      ...
      body.innerHTML = `
        <div class="node-full-card" style="padding:12px 16px;margin-bottom:8px">
          <div class="node-detail-name" style="font-size:20px" ${locodeAttr(n.name)}>${escapeHtml(n.name || '(unnamed)')}${dupBadge}</div>
          ${dupSection}
      ...
      // Scopes section (repeaters and rooms — both forward flood traffic and
      // already appear in transported_scopes, matching the line-684 gate
      // above) — observed-vs-declared region conformance
      if ((n.role === 'repeater' || n.role === 'room') && window.NodeScopes) {
  ...
  function renderDetail(panel, data) {
    ...
    panel.innerHTML = `
      ...
      <div class="node-detail">
        <div class="node-detail-name" ${locodeAttr(n.name)}>${escapeHtml(n.name || '(unnamed)')}${dupBadge}</div>
