...
func deriveHeardKeyGeo(direction string, routeType, payloadType int, hops []string, advertPubkey string, isAdvert bool, isDiscoverResp bool, discoverPubkey string, lat, lon float64, idx geoIndex) (string, int, string, bool) {
	if key, keylen, src, ok := deriveHeardKey(direction, routeType, payloadType, hops, advertPubkey, isAdvert, isDiscoverResp, discoverPubkey); ok {
...
func buildClientReceptionGeo(
	...
	heardKey, keylen, src, ok := deriveHeardKeyGeo(direction, routeType, payloadType, hops, advertPubkey, isAdvert, isDiscoverResp, discoverPubkey, lat, lon, idx)
	if !ok {
		return nil, false
	}
	return &ClientReception{
