...
// scope-repair is a one-off maintenance subcommand (#1609) that re-derives
// transmissions.scope_name for every transport-scoped row using the current
// matchScope, and corrects rows that were mis-derived by the old
// first-match logic. It never runs as part of normal ingestor startup — it
...
// corrects exactly the rows whose stored name is a non-empty region that
// re-derives to "" (an ambiguous multi-key match under the pre-#1609
// first-match bug). Every other disagreement between stored and re-derived
// state is reported as unexpected and left untouched — it cannot be
// explained by the bug this tool repairs, and guessing would repeat the
// original mistake.
//
...
// Running repairScopeNames(apply=true) twice in a row corrects nothing the
// second time: every row it just corrected now reports "" for both stored
// and re-derived state, which is the Unchanged case.
func repairScopeNames(db *sql.DB, regionKeys map[string][]byte, apply bool) (*scopeRepairReport, error) {
	...
	report := &scopeRepairReport{Applied: apply, CorrectedByOldName: map[string]int{}}
	type fix struct {
		id     int64
		newVal string
	}
...
func writeScopeRepairReport(w io.Writer, r *scopeRepairReport) {
	...
	if r.Applied {
		fmt.Fprintf(w, "  named after:  %d, unnamed (\"\") after:  %d\n", r.NamedBefore-r.CorrectedTotal, r.UnnamedBefore+r.CorrectedTotal)
	} else {
