...
func TestDeriveHeardKeyGeo_OneByteHopResolvesToGeoSrc(t *testing.T) {
	...
	k, l, src, ok := deriveHeardKeyGeo("rx", packetpath.RouteFlood, PayloadGRP_TXT, []string{"1a"}, "", false, false, "", geoRecLat, geoRecLon, idx)
...
func TestDeriveHeardKeyGeo_TwoCandidatesNotAttributed(t *testing.T) {
	...
	if _, _, _, ok := deriveHeardKeyGeo("rx", packetpath.RouteFlood, PayloadGRP_TXT, []string{"1a"}, "", false, false, "", geoRecLat, geoRecLon, idx); ok {
		t.Fatal("two in-range candidates must not be attributed")
	}
}
...
func TestDeriveHeardKeyGeo_MultiByteHopStillRxlog(t *testing.T) {
	k, l, src, ok := deriveHeardKeyGeo("rx", packetpath.RouteFlood, PayloadGRP_TXT, []string{"aa", "bbccdd"}, "", false, false, "", geoRecLat, geoRecLon, nil)
...
func TestDeriveHeardKeyGeo_AdvertUnaffected(t *testing.T) {
	...
	k, l, src, ok := deriveHeardKeyGeo("rx", packetpath.RouteFlood, PayloadADVERT, nil, strings.ToUpper(full), true, false, "", geoRecLat, geoRecLon, nil)
...
func TestDeriveHeardKeyGeo_DirectRouteNeverEntersGeoPath(t *testing.T) {
	full := geoTestPubkey("1a")
	idx := geoIndex{"1a": {{Pubkey: full, Lat: 51.10, Lon: 3.75}}}
	if _, _, _, ok := deriveHeardKeyGeo("rx", packetpath.RouteDirect, PayloadGRP_TXT, []string{"1a"}, "", false, false, "", geoRecLat, geoRecLon, idx); ok {
		t.Fatal("DIRECT route must never be attributed, even via the geo path")
	}
	if _, _, _, ok := deriveHeardKeyGeo("rx", packetpath.RouteTransportDirect, PayloadGRP_TXT, []string{"1a"}, "", false, false, "", geoRecLat, geoRecLon, idx); ok {
...
func TestDeriveHeardKeyGeo_TraceNeverEntersGeoPath(t *testing.T) {
	full := geoTestPubkey("1a")
	idx := geoIndex{"1a": {{Pubkey: full, Lat: 51.10, Lon: 3.75}}}
	if _, _, _, ok := deriveHeardKeyGeo("rx", packetpath.RouteFlood, PayloadTRACE, []string{"1a"}, "", false, false, "", geoRecLat, geoRecLon, idx); ok {
...
func TestBuildClientReceptionGeo_OneByteHopResolvesToGeoSrc(t *testing.T) {
	...
	idx := geoIndex{"1a": {{Pubkey: full, Lat: 51.10, Lon: 3.75}}}
	acc := 8.0
	rec, ok := buildClientReceptionGeo("companionpk", "rx", packetpath.RouteFlood, PayloadGRP_TXT, []string{"1a"}, "", false, false, "",
...
func TestBuildClientReceptionGeo_ValidationGuardsPreserved(t *testing.T) {
	full := geoTestPubkey("1a")
	idx := geoIndex{"1a": {{Pubkey: full, Lat: 51.10, Lon: 3.75}}}
	if _, ok := buildClientReceptionGeo("", "rx", packetpath.RouteFlood, PayloadGRP_TXT, []string{"1a"}, "", false, false, "",
		nil, nil, geoRecLat, geoRecLon, nil, "t", "t", idx); ok {
		t.Fatal("empty rxPubkey must be rejected")
	}
	if _, ok := buildClientReceptionGeo("c", "rx", packetpath.RouteFlood, PayloadGRP_TXT, []string{"1a"}, "", false, false, "",
