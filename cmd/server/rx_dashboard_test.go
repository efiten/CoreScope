...
func TestRxLeaderboard(t *testing.T) {
	...
	insRx(t, db, "compb", "aabbcc", recent, 51.05, 3.72)
	srv := &Server{db: db}
	...
	if len(obs) != 2 || obs[0].Pubkey != "compa" || obs[0].Receptions != 3 || obs[0].Nodes != 3 || obs[0].Name != "MyCompanion" {
		t.Fatalf("leaderboard: %+v", obs)
	}
}
