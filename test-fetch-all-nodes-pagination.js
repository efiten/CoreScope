 ...
 *
 * Bug: /api/nodes clamps ?limit to 500 (handleNodes, routes.go) and orders by
 * last_seen DESC. Every node-list consumer (map.js, live.js, analytics.js,
 * packets.js, area-map.html) issued a single ?limit=N fetch and trusted the
 * response as the full set, so a mesh with >500 nodes silently saw only the top
 * 500. A repeater that relays constantly but last self-advertised hours ago fell
 * outside that window and vanished from the map/live view even though it was
 * plainly alive.
 *
 * Fix: a shared app.js `fetchAllNodes()` helper pages through /api/nodes (500
 * per request) until a short page, deduping by public_key, and caps the result
 * at safetyCap. This test drives the REAL api()+fetchAllNodes against a mocked
 * fetch with a 500-per-page cap and the server's unreliable per-page `total`.
 * Pre-fix consumers saw 500; the helper surfaces the full set.
 */
'use strict';
const vm = require('vm');
const fs = require('fs');
